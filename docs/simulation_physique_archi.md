...
## Phase 3.2 — `ball.py` (créé le 23/09/2026) : trajectoire du ballon, tag-driven + repli par inférence

Deux blocages identifiés avant d'écrire `ball.py`, arbitrés par Olivier le même jour :

1. **`BallState` élargi**, pas dupliqué (`types.py`) : `vx`, `vy`, `vz` ajoutés avec défaut `0.0` (aucun appel existant cassé, tous en kwargs), `owner_id` conservé, `spin` fixé en radians/seconde (commentaire mis à jour, toujours non consommé par le rendu). `Keyframe.physics_tag: str | None = None` ajouté, DISTINCT de `tag` (narratif, jamais lu par un comportement) -- docstring de `Keyframe` explicite sur la différence. `to_json()` sérialise les deux nouveautés.
2. **Tag-driven hybride** (Modification A/B/C du brief) : `Template.physics_tags: tuple[tuple[float, str], ...] = ()` (nouveau champ, `templates.py`), câblé dans `build_from_template` -> `Keyframe.physics_tag`. **13 tags explicites, chirurgicaux** (pas de retaggage exhaustif) : un `"shot"` sur le dernier segment de CHACUN des 12 gabarits (structurel : c'est toujours le tir/la tête/la déviation finale), plus `"deflect"` sur `recuperation_haute` à t=0.3 (porteur `None`, pressing disputé). Repli (`animation.ball._resolve_physics_tag`) pour tout segment sans tag explicite : porteur `None` -> `deflect`, dernier segment -> `shot`, sinon -> `pass_ground`. Garde-fou AVANT d'appliquer les 13 tags : `tests/test_templates.py::TestTemplateStructureUnchangedByPhysicsTags` (empreinte SHA256 du JSON de chaque gabarit, `physics_tag` neutralisé, capturée avant modification) -- confirme qu'aucune position/tag narratif/ball_owner/ball_height n'a bougé.

**Point non résolu, laissé en suspens** : le segment `corner` 0.3->0.6 (livraison du corner, tag narratif "centre", porteur `None`) n'a PAS de `physics_tag` explicite -- ambiguïté entre `cross` (le tag narratif suggère un centre) et `deflect` (la règle générale "porteur `None`" le suggère aussi) jamais tranchée. **Découverte en écrivant `scripts/preview_ball.py`** : `kf[t_ratio=0.3].ball.(x,y)` et `kf[t_ratio=0.6].ball.(x,y)` sont EXACTEMENT identiques dans les données actuelles (`build_from_template` fait tenir le ballon sur la DERNIÈRE position connue quand `ball_owner` est `None`, plutôt que de dériver une position "en vol" distincte) -- donc quel que soit le tag choisi pour ce segment, `ball_state_at` déclenche aujourd'hui le cas limite "ballon immobile" et ne produit AUCUNE trajectoire visible. Corriger ça demanderait de toucher à la dérivation de position du ballon dans `build_from_template` lui-même, explicitement hors scope de ce brief ("aucun autre changement" que les `physics_tags` listés).

`animation/ball.py` : `ball_state_at(sequence, t) -> BallState`, point d'intégration UNIQUE dans `motion.interpolate` (peuple enfin `FrameState.ball`, toujours `None` avant ce jour). Cinq comportements (`pass_ground`, `pass_lob`, `shot`, `cross`, `deflect`), chacun une fonction pure `(start, end, s, duration[, seed_key]) -> BallState` -- toute amplitude "aléatoire" (courbure d'un tir, contrôle latéral d'un centre, perturbation d'une déviation) est un hash sha256 déterministe de `(event_ref, t de la keyframe de départ)`, jamais `random`. `ball_height`/`Keyframe.ball.z` (l'ancien champ) est IGNORÉ (ses valeurs n'ont jamais été calibrées en mètres) -- `ball_state_at` calcule sa hauteur en mètres depuis ses propres formules. Continuité garantie DANS un segment, PAS garantie d'un segment à l'autre quand le comportement change (documenté en tête de module, pas un chantier de spline globale C1 pour ce brief).

**Bug trouvé et corrigé en testant `deflect`** : la première version de la fenêtre de perturbation utilisait un simple sinus, dont la dérivée n'est PAS nulle à l'entrée de la fenêtre (t=0.85) -- un saut de vitesse instantané, détecté par `tests/test_ball.py::TestContinuityAndArrivalPerBehavior`. Remplacé par un cosinus surélevé (`0.5*(1-cos(2*pi*w))`), dérivée nulle aux DEUX bords de la fenêtre. **Tension résiduelle documentée, pas cachée** : même avec cette fenêtre lisse, l'amplitude (8% de la distance) sur une fenêtre aussi courte (15% de la durée) implique mathématiquement une vitesse de perturbation bien plus grande que la vitesse de base dès l'entrée dans la fenêtre -- tolérance de continuité élargie spécifiquement pour `deflect` dans les tests, avec la justification en commentaire.

`pass_lob` : comportement complet, testé unitairement, mais déclenché par AUCUN des 12 gabarits actuels (confirmé avec Olivier -- `debordement_centre_tete` est un `cross`/tête, pas un lob entre deux joueurs au sol ; disponible pour un futur gabarit).

`scripts/preview_ball.py` : 12 PNG dans `docs/previews/ball/` (trajectoire au sol colorée par comportement résolu + graphe z(t)), dessinés à la main avec Pillow (déjà une dépendance du projet) -- pas de nouvelle dépendance (matplotlib) ajoutée pour ce seul script.

**498 tests verts** (`tests/test_ball.py`, nouveau : continuité dt=1/60 + arrivée exacte + déterminisme pour les 5 comportements, un test par comportement, 3 cas limites, inférence de repli re-dérivée indépendamment sur les 12 gabarits réels).
